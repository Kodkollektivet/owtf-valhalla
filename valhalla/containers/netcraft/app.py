...
def run():
    ...
    execute = "python netcraft.py -H " + req.get('command')
...
def parse(results):
    ...
    :param results: Nmap result in a string
    :return: List wirh parsed data
    """
    ret_list = []
    res = results.split('\\n')
    for i in res:
...
def result():
    ...
    if not process:
        return jsonify({"status": 1, "response": "No running process"})
    ...
    return jsonify({"status": 0, "response": parse(str(res[0]))})
